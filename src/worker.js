/**
 * Cloudflare Worker for serving Docusaurus static site on /monitor route
 * This worker handles routing and serves static assets for testforce.uk/monitor
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle requests to /monitor route
    let pathname = url.pathname;
    
    // If the path starts with /monitor, remove it for internal routing
    if (pathname.startsWith('/monitor')) {
      pathname = pathname.replace('/monitor', '') || '/';
    } else {
      // Return 404 for non-/monitor routes to maintain consistency
      return new Response('Not Found - Please access via /monitor route', { status: 404 });
    }
